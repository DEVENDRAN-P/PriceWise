...
export default function CategoryDetailPage({ categoryName }: CategoryPageProps) {
  const { getItemsByCategory, addToCart, prices, shops } = useApp();
  const [, setLocation] = useLocation();

  const items = getItemsByCategory(categoryName);
  const categoryData = CATEGORY_CONTENT[categoryName as keyof typeof CATEGORY_CONTENT];
  const interactiveData = INTERACTIVE_FEATURES[categoryName as keyof typeof INTERACTIVE_FEATURES];
